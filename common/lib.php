...
function equella_getssotoken($course = null) {
    ...
    foreach($editingroles as $role) {
        ...
        if ($hassystemrole || $hascategoryrole || $hascourserole) {
            ...
            if (!empty($shareid)) {
                return equella_getssotoken_raw($USER->equella_userfield, $shareid, $CFG->{"equella_{$role->shortname}_sharedsecret"});
            }
        }
    }
    ...
    if (!empty($shareid)) {
        return equella_getssotoken_raw($USER->equella_userfield, $shareid, $CFG->equella_sharedsecret);
    }
}
